...
## Переполнение диска и нехватка памяти
Симптомы:
- `resource_monitor.sh` отправляет alert: диск > 85%, память > 92% или swap > 90%.
- Нехватка памяти проявляется раньше всего подвисанием графической сессии
  (`_windowserver` в топе по CPU) — контейнеры при этом ещё живы.
