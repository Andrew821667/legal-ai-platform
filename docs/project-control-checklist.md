...
- `[~]` Почистить import graph legacy; прямую связку `callbacks -> user` уже убрали, admin/runtime блок вынесен из `callbacks.py`, `database.py` уже сокращен до ~1430 строк, но модульная декомпозиция еще не закончена.
