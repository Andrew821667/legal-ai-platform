...
- `[x]` Стартовый flow упрощен до понятного first-touch entry-screen: `/start` и простые приветствия ведут в один стартовый экран с ясным объяснением, для кого сервис, с чего начать и какие действия доступны сразу; возвраты из personal-mode и post-consent сценарии больше не дублируют лишнее приветствие.
...
- `[x]` Runtime audit от `2026-03-12` пройден: внутренние legal routes (`/privacy`, `/terms`, `/user-agreement`, `/transborder-consent`, `/marketing-consent`, `/ai-policy`) отвечают `200`; критичные runtime secrets и public disclosure vars, включая `OPERATOR_INN` / `NEXT_PUBLIC_OPERATOR_INN`, заполнены. Незаполненными остаются только optional integration secrets (`API_KEY_WORKER`, `ALERT_BOT_TOKEN`, `DEEPSEEK_API_KEY`, `PERPLEXITY_API_KEY`, `GA4_CREDENTIALS`, `YM_ACCESS_TOKEN`, `GITHUB_TOKEN`, `SMTP_PASSWORD`).
