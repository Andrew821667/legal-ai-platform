...
# Узлы «белых списков» провайдер держит на отдельных подсетях, устойчивых к
# блокировкам, — их ставим в пул первыми. Порядок выдачи подписки при этом
# игнорируем: он меняется почти каждый цикл и сам по себе не значит ничего.
PRIORITY_MARKERS = ("белые списки", "whitelist", "white list")
# Заблокированный адрес отвечать на TCP перестаёт раньше, чем провайдер
...
def node_alive(addr, port):
    """TCP-проверка узла перед включением в пул."""
    try:
        with socket.create_connection((addr, int(port)), timeout=PROBE_TIMEOUT):
            return True
    except Exception:
        return False
...
def main():
    ...
    for e in entries:
        ...
        # Служебные заглушки провайдера приходят не только в названии узла,
        # но и адресом-пустышкой (app.deprecated) — фильтр по remarks их не
        # ловит, и такой узел занимает слот в пуле, а балансировщик тратит на
        # него попытки. Узел с нерезолвящимся адресом рабочим быть не может.
        _addr = v.get("address", "")
        if any(c.isalpha() for c in _addr):
            try:
                socket.gethostbyname(_addr)
            except Exception:
                log("skip unresolvable node address: %s" % _addr)
                continue
        key = (v["address"], v.get("port"))
        if key in seen:
            continue
        seen.add(key)
        priority = 0 if any(m in rem.lower() for m in PRIORITY_MARKERS) else 1
        candidates.append((priority, str(v["address"]), int(v.get("port") or 0), copy.deepcopy(ob)))

    if not candidates:
        log("WARN pool empty after filter; leaving balancer as-is")
        return

    # Живость проверяем параллельно: 9 узлов по три секунды последовательно
    # растянули бы синхронизацию на полминуты без всякой пользы.
    with ThreadPoolExecutor(max_workers=min(8, len(candidates))) as pool_exec:
        alive_flags = list(pool_exec.map(lambda c: node_alive(c[1], c[2]), candidates))
    alive = [c for c, ok in zip(candidates, alive_flags) if ok]
    if len(alive) != len(candidates):
        log("skipped %d unreachable nodes (blocked or rotated away)" % (len(candidates) - len(alive)))
    if not alive:
        # Все узлы недоступны — оставить прежний конфиг безопаснее, чем
        # выкатить заведомо пустой и оборвать канал совсем.
        log("WARN all nodes unreachable; leaving balancer as-is")
        return

    # Порядок детерминированный: белые списки первыми, дальше по адресу.
    # Из-за этого хэш конфига перестаёт меняться от одной лишь перестановки
    # узлов в выдаче подписки — а значит, балансировщик больше не
    # перезагружается каждые полчаса, обрывая боты на ровном месте.
    alive.sort(key=lambda c: (c[0], c[1], c[2]))

    pool, ips = [], []
    for priority, addr, port, ob in alive:
        ob["tag"] = "proxy-%d" % (len(pool) + 1)
        pool.append(ob)
        ips.append(addr)
    log("alive nodes: %d (priority whitelist: %d)" % (len(pool), sum(1 for c in alive if c[0] == 0)))
