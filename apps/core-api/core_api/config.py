...
class Settings(BaseSettings):
    ...
    # Обезличивание по сроку хранения (152-ФЗ, anonymization.py): клиенты без
    # договора — через год в архиве и не позже трёх лет с последней
    # активности (потолок из политики конфиденциальности); подписанный NDA —
    # через три года с подписания (исковая давность).
    # Выключено, пока владелец не посмотрит предпросмотр: обезличивание
    # необратимо, а в базе могут быть давние записи из прежней системы.
    ...
    # Настройкой — на случай, если закон его поменяет.
    npd_annual_limit_minor: int = 240_000_000
    # Просьба оценить работу через сутки после оплаты акта (client_reviews).
