...
class LLMNewsWriter:
    ...
    def _quality_gate_failure_reason(
        ...
        *,
        ...
        style_failure = LLMNewsWriter._human_style_failure_reason(normalized)
        if style_failure:
            return style_failure
        return None

    @staticmethod
    def _human_style_failure_reason(text: str) -> str | None:
        plain = html.unescape(re.sub(r"<[^>]+>", " ", text or ""))
        plain = re.sub(r"\s+", " ", plain).strip().lower()
        if not plain:
            return None

        hard_hits = [item for item in _ROBOT_STYLE_HARD_PATTERNS if item in plain]
        if hard_hits:
            return f"robot_style_phrase:{hard_hits[0]}"

        soft_hits = sum(plain.count(item) for item in _ROBOT_STYLE_SOFT_PATTERNS)
        negative_parallelism = len(
            re.findall(r"\bне\s+(?:просто|только)\b.{0,100}\b(?:а|но)\b", plain, flags=re.IGNORECASE)
        )
        sentence_starts = re.findall(r"(?:^|[.!?]\s+)(это|такая|такой|данн(?:ый|ая|ое))\b", plain)
        dash_count = plain.count("—")
        ...
        if soft_hits >= 3:
            ...
            score += 1
        if negative_parallelism >= 2:
            score += 2
        elif negative_parallelism == 1:
            score += 1
        if len(sentence_starts) >= 3:
            score += 1
        if dash_count >= 7 and len(plain) < 2600:
            score += 1

        if score >= 3:
            return f"robot_style_cluster:score={score}"
        return None
        ...
        *,
        ...
        if format_type == "weekly_review":
            ...
            base.update(
                {
                    "lead": (
                        "За неделю стало заметно: компании реже обсуждают возможности моделей сами по себе. "
                        "Теперь разговор начинается со сроков процесса, качества проверки и цены ошибки."
                    ),
                    "weekly_points": weekly_points[:10],
                    "what_happened": (
                        "В материалах недели повторялись вопросы масштабирования, интеграции и контроля качества. "
                        "Красивого демо уже недостаточно: заказчики хотят понимать, как инструмент переживет обычную нагрузку и кто проверит результат."
                    ),
                    "business_effect": (
                        "Юридической команде придется считать время цикла, возвраты и стоимость проверки на реальном потоке. "
                        "Если договоры или обращения не проходят быстрее, наличие ИИ-функции само по себе ничего не меняет."
                    ),
                    "legal_risks": (
                    ...
                    "conclusion": (
                        "Проверять стоит весь рабочий маршрут: входные данные, решение, контроль юриста и действие после проверки. "
                        "Иначе удачный пилот так и останется единичным экспериментом."
                    ),
                }
            )
