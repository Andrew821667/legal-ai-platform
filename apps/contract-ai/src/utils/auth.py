...
def show_upgrade_message(feature: str):
    """Показать сообщение о следующем этапе доступа"""
    st.warning(f"""
    ⚠️ **Функция "{feature}" недоступна в текущем режиме доступа**

    Для расширенного сценария нужен следующий этап запуска:
    - **Пилотный контур** - проверка на реальных документах
    - **Рабочий контур** - расширенный функционал и настройка под процесс
    """)
