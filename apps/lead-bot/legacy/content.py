...
def build_welcome_message(first_name: str) -> str:
    ...
    return (
        ...
        "Помогаю быстрее навести порядок в intake, договорах, комплаенсе и legal ops без лишней ручной работы.\n\n"
        "Можно начать с кнопок ниже: посмотреть услуги и ориентиры по бюджету, выбрать профиль задач или описать кейс в 1-2 предложениях.\n\n"
...
def build_workspace_text(
    ...
    return (
        ...
        "Просмотр меню и документов доступен сразу. Для персональной заявки и AI-разбора я сначала попрошу согласие на обработку данных.\n\n"
...
MENU_RESPONSES = {
    ...
    "menu_contract_ai": (
        "🧪 CONTRACT_AI_SYSTEM\n\n"
        "Флагманский договорный модуль Legal AI PRO:\n"
        "• быстрый демо-анализ договора\n"
        "• чек-лист ключевых рисков\n"
        "• образец AI-отчета по договору\n"
        "• маршрут к пилоту внедрения\n\n"
        "Для начала выберите один из форматов ниже в блоке lead-магнитов."
    ),
    "menu_leave_contact": (
        "📲 ОСТАВИТЬ КОНТАКТ\n\n"
...
LEAD_MAGNET_SELECTION_MESSAGES = {
    ...
    "sample_report": (
        "Отлично, отправим образец AI-отчета по договору.\n\n"
        "Укажите email, куда направить материал."
    ),
}
...
LEAD_MAGNET_SENT_MESSAGES = {
    ...
    "sample_report": (
        "✅ Готово, образец AI-отчета отправлен.\n\n"
        "Проверьте почту, включая папку «Спам»."
    ),
}
...
CONSENT_STEP_1_TEXT = (
    ...
    "• для AI-анализа действует отдельное согласие на трансграничную передачу\n\n"
    "Ваши права:\n"
...
CONSENT_TRANSBORDER_TEXT = (
    ...
    "Для AI-ответов сообщения отправляются во внешние LLM-сервисы (например, OpenAI-совместимые провайдеры).\n"
