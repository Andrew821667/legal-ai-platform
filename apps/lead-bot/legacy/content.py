...
def build_welcome_message(first_name: str) -> str:
    ...
    return (
        ...
        "<b>Legal AI PRO</b> помогает юристам и бизнесу быстрее работать с договорами, "
        "входящими юридическими запросами и повторяющейся рутиной.\n\n"
        "<b>Когда это полезно:</b>\n"
        "• долго согласуются договоры\n"
        "• теряются запросы от бизнеса\n"
        "• слишком много ручной переписки\n"
        "• нужен понятный первый шаг по автоматизации\n\n"
        "Можно начать без специальных терминов: выбрать профиль, посмотреть услуги и цены "
...
def build_workspace_text(
    ...
    if include_context_intro:
        intro_parts.append(
            "Legal AI PRO помогает разобраться, где автоматизация и ИИ "
            "могут ускорить юридическую работу без перегруза команды."
        )
    ...
    return (
        "<b>🧭 Рабочий стол</b>\n\n"
        f"{intro}"
        "Если вы здесь впервые, сначала выберите профиль услуг. "
        ...
        f"<b>Режим:</b> {_e(mode)}\n\n"
        "<b>Быстрый маршрут:</b>\n"
        "1. Выбрать профиль услуг\n"
        "2. Посмотреть услуги и ориентиры по бюджету\n"
        "3. Проверить договор или запросить консультацию\n\n"
        "Меню и документы доступны сразу. Для персональной заявки и ИИ-разбора я сначала попрошу согласие на обработку данных.\n\n"
