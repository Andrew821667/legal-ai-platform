...
def _platform_prices_text(platform_key: str) -> str:
    ...
    return (
        ...
        f"{bullets}\n\n"
        "Точный расчет зависит от объема задач, интеграций и требуемой скорости работы. Для персонального расчета нажмите «📞 Консультация».\n"
...
def legal_disclaimer_short_text() -> str:
    return "Важно: ответы бота носят информационный характер и не заменяют персональную юридическую консультацию."
...
def pdn_consent_required_text(action_label: str | None = None) -> str:
    action_prefix = (
        f"Чтобы перейти к «{action_label}», нужно согласие на обработку персональных данных."
        if action_label
    ...
    return (
        ...
        "После подтверждения станут доступны передача контакта, получение материалов и персональная заявка."
    )
...
def build_welcome_message(first_name: str) -> str:
    ...
    return (
        f"Здравствуйте, {name}.\n\n"
        "На связи ассистент Legal AI PRO.\n\n"
        "Если совсем коротко: мы помогаем юристам и бизнесу быстрее работать с договорами, входящими юридическими задачами "
        "и повторяющейся рутиной с помощью автоматизации и ИИ.\n\n"
        "Обычно к нам приходят с понятными прикладными задачами: долго согласуются договоры, теряются запросы от бизнеса, "
        "много ручной переписки или нужен понятный первый шаг по автоматизации.\n\n"
        "Если вы только знакомитесь с темой, это нормально. Можно начать без специальных терминов: "
        "посмотреть услуги, цены, выбрать свой профиль или просто описать проблему обычными словами.\n\n"
...
def build_business_welcome_message(first_name: str) -> str:
    return (
        ...
        "Для быстрого старта используйте кнопки ниже: "
        "«🧪 Проверить договор», «📞 Консультация», «📲 Оставить контакт»."
    )
...
def build_workspace_text(
    ...
    return (
        "🧭 РАБОЧИЙ СТОЛ\n\n"
        f"{intro}"
        "Если вы здесь впервые, начните с верхней кнопки «🎯 Профиль услуг». "
        "Так бот поймет, кто вы: руководитель бизнеса, юридический отдел компании или юрфирма.\n\n"
        f"Сейчас активен: {active_profile_label}\n"
        f"Режим: {mode}\n\n"
        "Что можно сделать прямо сейчас:\n"
        "• понять, чем мы вообще можем быть полезны\n"
        "• посмотреть услуги и ориентиры по бюджету\n"
        "• проверить договор или запросить консультацию\n"
        "• оставить контакт для связи с командой\n"
        "• открыть профиль и документы\n\n"
        "Просмотр меню и документов доступен сразу. Для персональной заявки и ИИ-разбора я сначала попрошу согласие на обработку данных.\n\n"
...
MENU_RESPONSES = {
    "menu_services": (
        ...
        "Это основные форматы работы. Если не уверены, какой из них ваш, начните с профиля услуг или консультации."
    ),
    "menu_prices": (
    ...
    "menu_help": (
        "❓ КАК Я ПОМОГАЮ:\n\n"
        "• объясняю простыми словами, что именно делает Legal AI PRO и в каких задачах помогает\n"
        "• помогаю понять, какой формат вам подходит\n"
        "• собираю контекст перед консультацией с командой\n\n"
        "Можно начать одной фразой, например:\n"
    ...
    "menu_contract_ai": (
        "🧪 ПРОВЕРКА ДОГОВОРА\n\n"
        "Contract_AI_System — это наш отдельный сервис для работы с договорами.\n\n"
        "Он помогает:\n"
        "• быстро сделать демонстрационный разбор договора\n"
        "• чек-лист ключевых рисков\n"
        "• пример отчета по договору\n"
        "• маршрут к пилоту внедрения\n\n"
        "Если не знаете, с чего начать, выберите демонстрационный разбор договора."
    ),
    "menu_leave_contact": (
        "📲 ОСТАВИТЬ КОНТАКТ\n\n"
...
CONSENT_STEP_1_TEXT = (
    ...
    "Для персональной заявки и связи по вашему запросу нужно согласие на обработку ПД:\n"
    ...
    f"Подробная версия политики: {config.PRIVACY_POLICY_URL}\n\n"
...
TRANSBORDER_REQUIRED_TEXT = (
    "⚠️ Для ИИ-анализа вашего кейса нужно согласие на трансграничную передачу данных.\n\n"
    "Без него доступны: меню, консультация и ручная передача запроса команде."
)


CONSENT_DENIED_TEXT = (
    "Понял. Без согласия на обработку ПД я не смогу оформить персональную заявку или связать вас с командой.\n\n"
