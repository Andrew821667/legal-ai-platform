...
import { CLIENT_ACCOUNT_COOKIE, CLIENT_PROFILE_COOKIE, CLIENT_SESSION_COOKIE } from "@/lib/client-session";
...
export async function POST(request: NextRequest) {
  ...
  response.cookies.set(CLIENT_ACCOUNT_COOKIE, "", { httpOnly: true, secure: true, sameSite: "lax", path: "/", maxAge: 0 });
  return response;
}
