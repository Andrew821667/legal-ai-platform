 ...
 *
 * Два независимых пути входа. Из inline-кнопки и кнопки меню Telegram даёт
 * подписанный initData — это основной путь, строгий и без послаблений. Его
 * нет вовсе вне Telegram (иконка на экране iPhone, обычная вкладка Safari) и
 * — так устроен Telegram — при запуске кнопкой reply-клавиатуры. Для них
 * остаётся куда более узкая, но настоящая дверь — куки, выданная через
 * /lawyer/login по токену от бота. Куки пробуем, только когда заголовок
 * initData отсутствует целиком: если он есть, но не прошёл проверку, это не
 * повод молча переключаться на запасной путь — это ошибка, о которой надо
 * сказать прямо.
 */
